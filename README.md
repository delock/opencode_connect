...
多用户模式允许管理员将 OpenCode 控制权临时授权给其他用户。同一时间只有一个人能控制 bot，保证清晰的命令链。

### 命令列表

| 命令 | 权限 | 说明 |
|------|------|------|
| `\delegate username` | 管理员 | 授权指定用户控制 OpenCode |
| `\revoke` | 管理员 | 撤销授权，收回控制权 |
| `\status` | 所有人 | 查看当前状态（实例ID、目录、控制者、Shell模式） |
...
|------|------|
| `\\` | 忽略此消息（用于频道内正常聊天） |
| `/` | 被拦截，显示 "Command mode not supported" |

### 使用场景

- 团队协作：管理员可临时授权同事使用 OpenCode
- 控制权清晰：避免多人同时操作造成混乱
